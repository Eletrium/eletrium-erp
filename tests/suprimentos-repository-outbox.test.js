...
(async function repositoryMapsIdsAndEnforcesIdempotency() {
  ...
  const graph = {
    query: async (list, filter) => {
      ...
      if (/duplicate/.test(filter || '')) return { complete: true, value: [{ id: 'prior', fields: { Reserva_ID: 'e2', Idempotency_Key: 'duplicate', Row_Version: 1 } }] };
  ...
  await assert.rejects(
    () => repo.appendReservationEvents([{ eventId: 'altered', idempotencyKey: 'duplicate', quantity: 99 }]),
    (error) => error.code === 'IDEMPOTENCY_EVENT_PAYLOAD_MISMATCH'
  );
